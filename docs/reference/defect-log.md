...
- **`Settings { EmptyView() }` 는 "Scene 이 없다"가 아니라 "열 수 있는 빈 창이 있다"다.** `App` 은
  Scene 을 최소 하나 요구하므로 메뉴바 앱은 관례적으로 빈 `Settings` 를 둔다(메뉴바는 Scene 이 아니라
  `NSStatusItem` 이 그린다). 그런데 `applicationShouldHandleReopen` 을 구현하지 않으면 **기본 reopen
  처리가 "창이 없으니 첫 Scene 을 열자"로 가서 그 빈 창을 띄운다** — 제목이 `<앱 이름> Settings` 인
  **빈 검은 창**이고, 사용자가 닫기 전까지 남는다. 이 앱은 LaunchAgent(KeepAlive)로 **상시 실행**이라
  "앱을 연다"는 행위가 거의 항상 새 실행이 아니라 reopen 이다 — 특히 **새 버전 설치 직후
  `/Applications` 에서 더블클릭하는 순간**이 그 경로라 업그레이드할 때마다 재현된다. 반환값 의미가
  뒤집혀 있으니 주의: `false` 가 "기본 동작을 하지 마라"다(Apple 문서). 가드는
  `MenuBarAppWindowTests` — reopen 응답뿐 아니라 **`WindowGroup` 이 생기지 않았는지**도 함께 본다
  (창을 여는 Scene 이 생기면 reopen 을 삼키는 것 자체가 버그가 된다).
- **"내가 만든 릴리스 이후에 보인다"가 "내 릴리스가 만들었다"는 아니다 — 먼저 diff 로 범위를 잘라라.**
  위 빈 창은 1.0.1 설치 직후에 처음 보였지만 `Settings { EmptyView() }` 는 **최초 커밋부터** 있던
  상류 코드였고, `git diff v1.0.0..v1.0.1 -- Sources/` 는 UI 파일을 **하나도** 포함하지 않았다.
  바뀐 것은 코드가 아니라 **사용자의 행동**이다: 업그레이드는 사람이 앱을 더블클릭하게 만들고, 그게
  평소엔 안 밟는 reopen 경로를 밟는다. 잠복 결함은 "언제 심겼나"가 아니라 **"무엇이 그 경로를 처음
  밟게 했나"**로 추적한다.
